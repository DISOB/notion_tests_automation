...
@allure.parent_suite("pages")
@allure.suite("schemabased")
def test_update_page_properties_validation_error(client):
    pages = Pages(client.NOTION_API_BASE_URL)
    page_id = client.PAGE_TO_UPDATE_ID
    random_number = random.randint(1000, 9999)
    new_title = f"Page to update. Updated Page Title {random_number}"
...
@allure.parent_suite("pages")
@allure.suite("schemabased")
def test_update_page_properties_invalid_parent_page_id(client):
    pages = Pages(client.NOTION_API_BASE_URL)
    ...
    response = pages.update_page(notion_api_token=client.NOTION_API_TOKEN, page_id=invalid_page_id, properties=properties)
