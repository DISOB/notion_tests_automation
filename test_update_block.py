...
@allure.parent_suite("blocks")
@allure.suite("schemabased")
def test_update_block_invalid_parent_page_id(client):
    ...
    response = blocks.update_block(notion_api_token=client.NOTION_API_TOKEN, block_id=invalid_page_id, rich_text=rich_text)
...
@allure.parent_suite("blocks")
@allure.suite("schemabased")
def test_update_block_validation_error(client):
    blocks = Blocks(client.NOTION_API_BASE_URL)
    random_number = random.randint(1000, 9999)
    rich_text = [{
        "text": {"contnt": f"Block id here ({random_number})"}
    }]
    response = blocks.update_block(notion_api_token=client.NOTION_API_TOKEN, block_id=client.BLOCK_ID, rich_text=rich_text)
